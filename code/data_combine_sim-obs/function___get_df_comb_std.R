get_df_comb_std <- function(target_var, 
  ...
  for(ifile in files_obs){
    ...
    # select and combine
    df_dum <- df_obs %>% 
      select(x, y, year, month, varDFname_obs ) %>% # MP remove matches for Evapotranspiration matches(varDFname_sim)
      inner_join(df_sim %>% select(x, y, year, month, varDFname_sim), # MP remove matches for Evapotranspiration matches(varDFname_sim)
                 by = c("x", "y", "month", "year")) %>% 
      rename(obs = varDFname_obs, sim = varDFname_sim ) # MP remove matches for Evapotranspiration matches(varDFname_sim)
