...
# ########################################################
# Title         : 
# Description   : This code opens the .nc files and exports the data to dataframes with 0.05, 0.1 and 0.25 res
#                 the dataframes will cover the full time period 
#                 just need to change 01 025 and the name and the year to run
...
var <- 'LAI_theia_025'             # 025 and 01 # ET_GLEAM SM_ERA5l SM_sat SM_ERA5l_01_2009.nc LAI_theia_01_1998.nc
root_data   <- '/mnt/cidstorage/cidportal/data/cid-bulk22/Shared/projectData/COPERNICUSII/data/theia/data/nc_for_df/'
output_path <- '/mnt/cidstorage/cidportal/data/cid-bulk22/Shared/projectData/COPERNICUSII/data/theia/data/dataframe/'
#script_info <-'albedo_GEE'                 # 
start_year <- 2009 ; end_year <- 2019 #18      # just hardcode year range for now
...
sf <- list( ET_GLEAM = 1, ET_ERA5l = -1000,  SM_sat = 1, SM_ERA5l = 1, SM_ERA5l = 1, LAI_theia = 1, LAI_ERA5sl = 1, albedo_GEE = 100000, albedo_ERA5l = 1 ) # 
...
for(year_i in start_year:end_year){
  ...
  for (variable in names(ncin$var) ){
    ...
    df_variable <- as.data.frame(r, xy = T, na.rm = T, long = T)
  ...
  print(names(df))
  # save dataframe
  df <- df %>% filter(!is.na(LAI))
  save( df , file=paste0(output_path, 'df_', var, '_', year_i,'.RData') )
