...
var <- 'albedo_MCD43C3_025'             # 025 and 01 # ET_GLEAM SM_ERA5l SM_sat SM_ERA5l_01_2009.nc LAI_theia_01_1998.nc
root_data   <- '/mnt/cidstorage/cidportal/data/cid-bulk22/Shared/projectData/COPERNICUSII/data/NASA_albedo/MCD43C3/data/4_monMean/'
output_path <- '/mnt/cidstorage/cidportal/data/cid-bulk22/Shared/projectData/COPERNICUSII/data/NASA_albedo/MCD43C3/data/dataframe/'
#script_info <-'albedo_GEE'                 # 
start_year <- 2003 ; end_year <- 2018 #18      # just hardcode year range for now
...
var_data_type <- paste0(  str_split(var, '_' )[[1]][2]) # e.g. ET_ERA5l
...
albedo_list <- c('Albedo_WSA_vis', 'Albedo_BSA_nir', 'Albedo_BSA_vis', 'Albedo_WSA_nir')

#loop over the different albedos
for (jj in 1:length(albedo_list) ){ # jj <- 1
...
for(year_i in start_year:end_year){
  ...
  file <- paste0(root_data, var_name, '_', var_res, '_', year_i, '.nc')
  ...
  for (variable in names(ncin$var) ){
    ...
    if(variable != albedo_type & variable != 'Percent_Snow' ){print(paste('skip : ',variable)) ; next}
    ...
    names(df_variable)[names(df_variable) == 'value'] <- variable
    ...
    print(names(df))
    df$value.x <- NULL ; df$value.y <- NULL
    print(names(df))
  ...
  # save dataframe
  print('save df')
  var_albedo_type <- paste0(  tolower(str_split(albedo_type, '_' )[[1]][2]), '_', str_split(albedo_type, '_' )[[1]][3] ) 
  save( df , file=paste0(output_path, 'df_', var_name, '_', var_res, '_', var_albedo_type, '_', year_i,'.RData') )
