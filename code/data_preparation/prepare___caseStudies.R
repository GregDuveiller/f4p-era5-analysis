...
for( j in 1:dim(df_hw)[1] ){
  case_study <- row.names(df_hw)[j]
...
for (i in 1:length(var_list)){ # i <- 1
  variable_i <- var_list[i] ; print(variable_i)
  input_file <- paste0('df_comb___',variable_i,'.RData')
  if(  grepl( 'MCD43C3', variable_i, fixed = TRUE)) { variable_i <- gsub('_MCD43C3', '', variable_i, fixed = TRUE)}
  if(  grepl( 'MCD43GF', variable_i, fixed = TRUE)) { variable_i <- gsub('_MCD43GF', '', variable_i, fixed = TRUE)}
