...
# preparation
fig.format <- 'png'
fig.path <- 'results/hysteresis_LAI_albedo'
dir.create(fig.path)
...
df_ALB_comb <- temp_agr_con ; varname <- 'Albedo'

by_vctr <- c("time", "t2.clim.bin", "sm.clim.bin")

df_all <- left_join(by = by_vctr, suffix = c("_LAI", "_ALB"),
                    x = df_LAI_comb, y = df_ALB_comb) %>%
  select(by_vctr, dif_mu_LAI, dif_mu_ALB, N_LAI, N_ALB) %>%   # 
  rename(y = dif_mu_ALB, x = dif_mu_LAI, ny = N_ALB, nx = N_LAI) %>%
...
g_all_base <- ggplot(df_all) +
  ...
  coord_cartesian(xlim = c(-1, 1.5), ylim = c(-0.03, 0.03)) + 
